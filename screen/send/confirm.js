          ...
          if (this.state.payjoinUrl.includes('.onion')) {
            ...
            const customPayjoinRequester = {
              requestPayjoin: async function (psbt: Psbt) {
                console.log('=====requesting payjoin with psbt:', psbt.toBase64());
                ...
                console.log('=====got torResponse.body');
    ...
    return (
      <>
        <View style={styles.valueWrap}>
          ...
          <Text style={styles.valueUnit}>{' ' + loc.units[BitcoinUnit.CHESS]}</Text>
        </View>
        ...
        <View style={styles.cardBottom}>
          <BlueCard>
            <Text style={styles.cardText} testID="TransactionFee">
              {loc.send.create_fee}: {formatBalance(this.state.feeSatoshi, BitcoinUnit.CHESS)} (
